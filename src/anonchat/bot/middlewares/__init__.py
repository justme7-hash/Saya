...
class RateLimitMiddleware(BaseMiddleware):
    ...
    **نکته مهم:** این میدل‌ور فقط برای ``message`` اجرا می‌شود، نه ``callback_query``.
    دکمه‌های شیشه‌ای (inline) نباید Rate Limit شوند چون کاربر با کلیک روی آن‌ها
    پیام نمی‌فرستد — فقط درخواست callback می‌دهد. اگر Rate Limit برای callback_query
    هم اجرا شود، کاربر بعد از چند کلیک روی دکمه‌ها بدون هیچ پیامی مسدود می‌شود.
    """
    ...
    async def __call__(
        ...
        # فقط برای message اجرا شود، نه callback_query
