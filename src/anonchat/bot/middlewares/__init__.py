...
class BanCheckMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        async with container.session() as session:
            ...
            if ban is not None:
                # مقادیر لازم را قبل از بسته‌شدن نشست استخراج می‌کنیم
        ...
        if ban is not None:
            if isinstance(event, Update) and event.message:
                ...
                await event.message.answer(
                    t("banned_message", locale).format(reason=reason)
                    + until_text
                    + kind
                )
            return None
    ...
    async def __call__(
        ...
        async with container.session() as session:
            ...
            if db_user is not None:
                # مقادیر لازم را قبل از بسته‌شدن نشست استخراج می‌کنیم
                language = db_user.language or container.settings.default_locale
