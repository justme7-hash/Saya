...
from anonchat.bot.utils import reset_user_state, safe_answer, safe_edit_reply_markup, safe_edit_text
...
async def privacy_menu(callback: CallbackQuery) -> None:
    ...
    async with container.session() as session:
        ...
        settings = {
            ...
            "hide_from_search": getattr(db_user, "hide_from_search", False) if db_user else False,
        }
    # باگ: edit_text روی پیام غیرمتنی خطا می‌داد؛ از safe_edit_text استفاده می‌کنیم.
    await safe_edit_text(
        callback,
...
async def toggle_privacy(callback: CallbackQuery) -> None:
    ...
    برای hide_from_search، منطق برعکس است: دکمه ✅ یعنی «نمایش در جستجو»
    (hide=False). وقتی کاربر کلیک می‌کند، hide را toggle می‌کنیم.
    """
    ...
    async with container.session() as session:
        ...
        if field == "hide_from_search":
            # منطق برعکس: toggle کن
            current = getattr(db_user, "hide_from_search", False)
            new_value = not current
            await user_repo.update(db_user, hide_from_search=new_value)
        else:
