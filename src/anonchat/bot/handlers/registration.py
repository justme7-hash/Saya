...
async def _get_locale(container, telegram_id: int) -> str:
    """دریافت زبان کاربر با نشست امن."""
...
async def process_nickname(message: Message, state: FSMContext) -> None:
    """پردازش نام مستعار."""
    container = get_container()
    locale = await _get_locale(container, message.from_user.id)
...
async def process_gender(callback: CallbackQuery, state: FSMContext) -> None:
    """پردازش جنسیت."""
    container = get_container()
    locale = await _get_locale(container, callback.from_user.id)
