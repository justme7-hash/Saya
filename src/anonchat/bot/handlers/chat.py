...
async def end_chat(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    # پیام پایان گفتگو + منوی اصلی
    await message.answer(t("chat_ended", locale), reply_markup=main_menu(locale))
    # پیام جداگانه برای امتیازدهی (با session_id در callback_data)
    # تا کاربر بتواند به این گفتگو امتیاز بدهد.
    await message.answer(
        t("chat_rate_prompt", locale),
        reply_markup=rating_keyboard(locale, session_id),
    )

    # اطلاع به شریک + نمایش کیبورد امتیازدهی به او نیز
    if partner_tg:
        bot = message.bot
        async with container.session() as session:
            user_repo = container.user_repo_with(session)
        ...
        try:
            ...
            # شریک هم می‌تواند به گفتگو امتیاز بدهد
            await bot.send_message(  # type: ignore[union-attr]
                partner_tg,
                t("chat_rate_prompt", partner_locale),
                reply_markup=rating_keyboard(partner_locale, session_id),
            )
...
# ---------------------------------------------------------------------------
#  امتیازدهی به گفتگو (بعد از پایان)
# ---------------------------------------------------------------------------

@router.callback_query(F.data.startswith("rate_"))
async def process_rating(callback: CallbackQuery) -> None:
    """پردازش امتیازدهی به گفتگو.

    فرمت ``callback_data``: ``rate_<rating>_<session_id>``.
    از ``callback_data`` به‌جای state استفاده می‌کنیم چون ``end_chat`` در پایان
    state را پاک می‌کند و ذخیره‌ی ``session_id`` در state قابل‌اعتماد نیست.
    """
    parts = callback.data.split("_")  # type: ignore[union-attr]  # مثال: ["rate", "5", "123"]
    if len(parts) != 3:
    ...
    try:
        await container.chat_service.rate_chat(
            callback.from_user.id, session_id, rating
        )
    except Exception as exc:
        _log.error("chat.rate_failed", error=str(exc))
        await safe_answer(callback, t("error_generic", locale))
        return

    # جایگزینی کیبورد با پیام تشکر
    await safe_edit_text(callback, t("chat_rate_thanks", locale))
    await safe_answer(callback)
