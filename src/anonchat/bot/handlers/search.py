...
@router.callback_query(F.data == "search_gender")
...
@router.callback_query(F.data == "search_language")
...
async def _perform_search(callback: CallbackQuery, state: FSMContext, criteria: SearchCriteriaDTO) -> None:
    ...
    try:
        ...
        try:
            ...
            _log.error(
                "search.perform_callback_failed",
                user_id=callback.from_user.id,
                error=str(exc),
            )
            import traceback
            _log.error("search.traceback", traceback=traceback.format_exc())
            try:
                await callback.message.answer(t("error_generic", locale))  # type: ignore[attr-defined]
            except Exception:
                pass
    finally:
        # مهم: callback.answer باید همیشه صدا زده شود
        try:
            await callback.answer()
        except Exception:
            pass
