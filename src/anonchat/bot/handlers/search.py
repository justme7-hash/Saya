...
async def cancel_search_handler(message_or_callback, state: FSMContext) -> None:
    """لغو جستجو."""
    container = get_container()
    ...
    if hasattr(message_or_callback, "message"):
        await message_or_callback.message.answer(  # type: ignore[attr-defined]
            t("search_cancelled", locale),
            reply_markup=main_menu(locale),
        )
    await message_or_callback.answer() if hasattr(message_or_callback, "answer") else None
