...
class StatsService:
    ...
    async def get_leaderboard(self, metric: str = "xp", limit: int = 10) -> list[dict]:
        """جدول رتبه‌بندی.

        مقادیر را داخل نشست به dict تبدیل می‌کند تا از DetachedInstanceError
        پس از بسته‌شدن نشست در هندلر جلوگیری شود.
        """
        async with self._container.session() as session:
            stats_repo = self._container.stats_repo_with(session)
            users = await stats_repo.get_top_users(metric=metric, limit=limit)
            # مقادیر مورد نیاز را داخل نشست به‌صورت dict استخراج می‌کنیم
            return [
                {
