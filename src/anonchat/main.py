...
async def ensure_tables_exist() -> None:
    """اطمینان از وجود جداول — fallback اگر مهاجرت شکست خورد.

    بررسی می‌کند که آیا جدول ``users`` وجود دارد؛ اگر نه، تمام
    جداول را از مدل‌ها می‌سازد.
    """
    try:
        container = get_container()
        from sqlalchemy import inspect

        # استفاده از engine برای inspect (نه session)
        ...
        if not table_names or "users" not in table_names:
            ...
            _log.info("main.tables_created_fallback")
        else:
